...
export default function GroupPrices({ groupedIds,
                                      tabTitles,
                                      lang,
                                      url,
                                      PromotionsTitle,
    ...
    const [tabType, setTabType] = useState("tab-0"); // Default to the first tab title
  ...
  useEffect(() => {
    const fetchAllGroups = async () => {
      try {
        ...
        const promises = groupedIds.map(async (ids) => {
    ...
    const handlePlanSelect = (plan) => {
        ...
        document.querySelector('.group-prices-component').dispatchEvent(event);
    };
